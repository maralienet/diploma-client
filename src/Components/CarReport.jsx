...
function CarReport() {
    ...
    function saveReport(isPeriod) {
        if (isPeriod) {
            axios.get(`https://diploma-server-1.onrender.com/pdf/cars/period/${dates[0]}/${dates[1]}`, { responseType: 'blob' })
                .then(response => {
            ...
            axios.get(`https://diploma-server-1.onrender.com/pdf/cars/month`, { responseType: 'blob' })
                .then(response => {
                    const url = window.URL.createObjectURL(response.data);
                    const a = document.createElement('a');
                    a.href = url;
